# -*- coding: utf-8 -*-
"""
Kenshi Shop - Final Professional Version

Features:
- ... (all existing features)
- NEW: Admins can now delete products, which also removes associated files from the server.
- NEW: Admins can mark orders as "proof" to be displayed on a public proofs page.
- NEW: Security improvement - credentials are now loaded from environment variables.
"""
...
# --- Core Config ---
app.config.update(
    # RENDER DEPLOYMENT: The DATA_FOLDER is now set to a persistent disk mount path.
    ...
    DATA_FOLDER=os.environ.get('DATA_FOLDER_PATH', 'data/'), # Use environment variable for Render disk
...
TELEGRAM_BOT_TOKEN = os.environ.get('TELEGRAM_BOT_TOKEN', "8280489753:AAFVUl8FhPj3qK2Aa8h_Q2N87K1j9jPOPEo")
ADMIN_TELEGRAM_CHAT_ID = os.environ.get('ADMIN_TELEGRAM_CHAT_ID', "8252162481")
...
for folder_key in [
    'UPLOAD_FOLDER', 'SECURE_FILES_FOLDER', 'TEMP_DOWNLOAD_FOLDER', 'RECEIPT_FOLDER', 'DATA_FOLDER'
]:
...
def approve_order(order_id):
    ...
    # Mark as proof if checkbox is checked
...
def run_bot():
    """Function to run the bot listener in a separate thread."""
...
if __name__ == '__main__':
    load_data()
    # On Render, the bot will be run in a separate "Background Worker" process.
    # This threading is for local development.
    if os.environ.get("RENDER") is None:
      bot_thread = threading.Thread(target=run_bot)
      bot_thread.daemon = True
      bot_thread.start()
    
    # Use '0.0.0.0' to be accessible in a container. Debug must be False for production.
